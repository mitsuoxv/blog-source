...
tiny_model2 <- function(population_growth, productivity_growth, mobilize, tax_income_rate, tax_consumption_rate, saving_rate = NULL) {
  ...
  by_period <- by_period %>% 
    mutate(
      ...
      after_tax_saving_rate = consump_retired / (GDP - tax_working)
    )
...
He explains government debt situation by comparing to household debt. I feel uneasy, because government debt creates mostly both credit and debt in house (a country), while household debt creates, well, only debt in house.
