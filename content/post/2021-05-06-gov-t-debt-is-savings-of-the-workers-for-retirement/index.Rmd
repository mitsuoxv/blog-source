...
## Is gov't debt a burden for the future generation?
...
Simon Wren-Lewis says yes. "Government debt can be a burden on future generations." ([here](https://mainlymacro.blogspot.com/2012/10/the-burden-of-government-debt.html))
...
tiny_model <- function(population_growth, productivity_growth, saving_rate = NULL) {
  by_generation <- tibble(
    ...
    rebirth_ratio = c(population_growth, NA)
  )
...
Below is a rough image of population changes in Japan. Period 4 is 2000-2030, Generation 4 is retired and Generation 5 is working. Although I am in the middle of Generation 4 and 5 in reality, let us read below as if we are Generation 5.

I assume no productivity growth. And I assume Generation n and Generation n-1 agree to share the produced by population ratio so that each person can consume equally in each Period. So government debt to GDP ratio, which is the same as the saving rate and the ratio of consumption by the retired to total consumption by definition of this model, is exactly same as population ratio of the retired in each Period.
...
model3$by_generation %>% 
  select(Generation, population, rebirth_ratio) %>% 
  knitr::kable(
...
Generation 5 saved 1953 yen in Period 4, but can consume only 1385 in Period 5. We can interpret this as Generation 4 earned negative interest rate, -1.14 percent.
...
# Rebirth ratio over Generation, 30 years:
population_growth <- c(rep(1.3, 3), 0.8, rep(0.65, 2))
...
This happens because I assume that productivity stops to grow from Period 5 when Generation 6 works, and that population rebirth ratio over Generation by Generation 4 and 5 is less than 1.
